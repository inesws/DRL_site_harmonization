...
save_results = "/home/isampaio/Desktop/Ines/DRL_site/results/with_fc_z_sex_xcov/"
...
for r in range(0,2):
    ...
    for i, (train_ind, val_ind) in enumerate(skf.split(stacked_FC_matrices,labels.iloc[:,0].values)):
        ...
        y_train_sex_labels=cov_train[:,1].astype('int').reshape((-1,))
        y_val_sex_labels=cov_val[:,1].astype('int').reshape((-1,))
        y_train_sex=tf.keras.utils.to_categorical(y_train_sex_labels, num_classes=2)
        y_val_sex=tf.keras.utils.to_categorical(y_val_sex_labels, num_classes=2)
        num_classes = 2
        ...
        history = autoencoder_2.fit(x=[X_train, env_train], y=y_train_sex,
                        epochs=epochs,
                        batch_size=batch,
                        shuffle=True, 
                        validation_data=([X_val, env_val], y_val_sex), callbacks=[callback])
        ...
        fc_train_encoded, h1_train, y_train_sex_pred = FC_encoder.predict(X_train)
        env_train_encoded = ENV_encoder.predict(env_train)
        z_train = fusion.predict([fc_train_encoded, env_train_encoded])      
        
        fc_train_decoded = fc_decoder.predict([z_train, y_train_sex_pred] )
        
        fc_val_encoded,  h1_val , y_val_sex_pred = FC_encoder.predict(X_val)
        env_val_encoded = ENV_encoder.predict(env_val)
        z_val = fusion.predict([fc_val_encoded, env_val_encoded])      
        
        fc_val_decoded = fc_decoder.predict([z_val, y_val_sex_pred])
        
        #____Sex classification based on softmax layer
      
        fold_results.loc[i,'softmax_sex_auc_roc'] = roc_auc_score(y_val_sex_labels, y_val_sex_pred[:,1] )

        y_val_sex_pred_class = pd.DataFrame(y_val_sex_pred).idxmax(axis=1).values
        fold_results.loc[i,'softmax_sex_f1'] = f1_score(y_val_sex_labels, y_val_sex_pred_class)
