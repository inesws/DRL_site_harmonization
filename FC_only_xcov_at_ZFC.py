...
fold_results = pd.DataFrame(np.zeros((11,22)), columns=['epochs','train_loss', 'val_loss', 'val_re_rmse',
...
for r in range(0,2):
    ...
    for i, (train_ind, val_ind) in enumerate(skf.split(stacked_FC_matrices,labels.iloc[:,0].values)):
        ...
        z_merged = Dense(128, activation='selu', kernel_initializer="lecun_normal")(fc_input)
        ...
        class AE(tf.keras.Model):
            def __init__(self, FC_encoder, fc_decoder, n_batch_size, **kwargs):
            ...
            def train_step(self, data):
                ...
                with tf.GradientTape() as tape: # foward propagation
                    ...
                    fc_z, h1, y_pred = self.FC_encoder(x, training=True)
                    z = fc_z
                    ...
                    fc_reconstruction_loss=re_loss(x, FC_reconstruction) 
                    ...
                    alpha=1
                    beta=15
                    total_loss = alpha*fc_reconstruction_loss + gamma*xcov_loss
            ...
            def test_step(self, data):
                ...
                fc_z, h1, y_pred = self.FC_encoder(x, training=False)
                z = fc_z
        ...
        autoencoder_2 = AE(FC_encoder, fc_decoder, batch)
        ...
        history = autoencoder_2.fit(x=X_train, y=y_train_multiclass,
                        epochs=epochs,
                        batch_size=batch,
                        shuffle=True, 
        ...
        dt = DecisionTreeRegressor( random_state=0)
        vc = VotingRegressor(estimators=[('knn', knn), ('svm', svr ), ('dt', dt)])
        vc.fit(fc_train_encoded,  train_env_risk)
        y_val_pred = vc.predict(fc_val_encoded)
        fold_results.loc[i,'fc_z_ERS_rmse'] = mean_squared_error(val_env_risk.ravel(), y_val_pred, squared=False)
