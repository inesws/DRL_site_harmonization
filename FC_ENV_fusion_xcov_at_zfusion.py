# -*- coding: utf-8 -*-
"""
Created on Thu Jun 27 00:16:17 2024
...
for r in range(0,2):
    ...
    for i, (train_ind, val_ind) in enumerate(skf.split(stacked_FC_matrices,labels.iloc[:,0].values)):
        ...
        outputs = [z_merged, h1_layer, supervised_output]
        inputs = [fc_input,env_input]
        fusion = Model(inputs, outputs, name = 'fusion_branch')
        fusion.summary()
        ...
        inputs = [latent_inputs, y_hat_inputs]
        env_decoder = Model(inputs, env_decoded, name='env_decoded')
        env_decoder.summary()
        ...
        class AE(tf.keras.Model):
            ...
            def train_step(self, data):
                ...
                with tf.GradientTape() as tape: # foward propagation
                    ...
                    z, h1_layer,y_pred = self.fusion([fc_z,env_z], training=True)
            ...
            def test_step(self, data):
                ...
                z, h1_layer, y_pred = self.fusion([fc_z,env_z], training=False)
        ...
        z_train, h1_train, y_train_sites_pred = fusion.predict([fc_train_encoded, env_train_encoded])      
        ...
        z_val, h1_val , y_val_sites_pred = fusion.predict([fc_val_encoded, env_val_encoded])      
        ...
        fold_results.to_csv(os.path.join(save_results, "FC_ENV_fusion_xcov_at_z2.csv"))



fold_results.iloc[i+1,:]=fold_results.iloc[0:10,:].mean(axis=0)

fold_results.T.to_csv(os.path.join(save_results, "FC_ENV_fusion_xcov_at_z2.csv"))
