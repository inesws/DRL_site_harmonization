...
save_results = "results/with_fc_z_site_xcov/"
...
for r in range(0,2):
    ...
    for i, (train_ind, val_ind) in enumerate(skf.split(stacked_FC_matrices,labels.iloc[:,0].values)):
        ...
        ## SITE AND SEX CLASSIFICATION - Dual Disentanglement

        # Site labels (multiclass)
        y_train_site_multiclass=cov_train[:,3:].copy()
        y_val_site_multiclass=cov_val[:,3:].copy()
        num_classes_site=y_train_site_multiclass.shape[1]
        ...
        x=Dropout(0.5)(x)
        supervised_output_site = Dense(num_classes_site, activation='softmax', kernel_regularizer=l2(0.01))(x)
        
        # Sex disentanglement block
        x2 = tf.keras.layers.Flatten()(h1_layer)
        x2=BatchNormalization()(x2)
        x2=Dropout(0.5)(x2)
        supervised_output_sex = Dense(num_classes_sex, activation='softmax', kernel_regularizer=l2(0.01))(x2)
        
        # Loss functions for both variables
        loss_sup_site = tf.keras.losses.CategoricalCrossentropy()
        loss_sup_sex = tf.keras.losses.CategoricalCrossentropy()

        outputs=[fc_flatten, h1_layer, supervised_output_site, supervised_output_sex]
        ...
        y_hat_inputs_site = Input(shape=(num_classes_site,), name='y_site_inputs')
        y_hat_inputs_sex = Input(shape=(num_classes_sex,), name='y_sex_inputs')
        concat_inputs = Concatenate(axis=1)([latent_inputs, y_hat_inputs_site, y_hat_inputs_sex])
        ...
        inputs = [latent_inputs, y_hat_inputs_site, y_hat_inputs_sex]
        ...
        class AE(tf.keras.Model):
            def __init__(self, FC_encoder,fc_decoder, ENV_encoder, env_decoder, fusion, n_batch_size, **kwargs):
                ...
                self.clf_loss_tracker = tf.keras.metrics.Mean(name='clf_loss_site')
                self.clf_loss_sex_tracker = tf.keras.metrics.Mean(name='clf_loss_sex')
            ...
            def train_step(self, data):
                ...
                with tf.GradientTape() as tape: # foward propagation
                    ...
                    fc_z, h1, y_pred_site, y_pred_sex = self.FC_encoder(fc, training=True)
                    env_z = self.ENV_encoder(env, training=True)
                    z = self.fusion([fc_z,env_z], training=True)
                    
                    FC_reconstruction = self.fc_decoder([z, y_pred_site, y_pred_sex], training=True)
                    ...
                    clf_loss_site=loss_sup_site(y_site, y_pred_site)
                    clf_loss_sex=loss_sup_sex(y_sex, y_pred_sex)
                    clf_loss = clf_loss_site + clf_loss_sex
                          
                    y_pred_site_mean_over_batches=tf.reduce_mean(y_pred_site,axis=0, keepdims=True)
                    ...
                    z_conf_flatten_site = (y_pred_site-y_pred_site_mean_over_batches)
                ...
                print("Shape of y_site in train_step:", y_site.shape, "Shape of y_sex in train_step:", y_sex.shape)
                ...
                self.clf_loss_tracker.update_state(clf_loss_site)
                self.clf_loss_sex_tracker.update_state(clf_loss_sex)
                ...
                return {
                    ...
                    "clf_loss_site": self.clf_loss_tracker.result(),
                    "clf_loss_sex": self.clf_loss_sex_tracker.result(),
            ...
            def test_step(self, data):
                ...
                fc_z, h1, y_pred_site, y_pred_sex = self.FC_encoder(fc, training=False)
                env_z = self.ENV_encoder(env, training=False)
                z = self.fusion([fc_z,env_z], training=False)
                    
                FC_reconstruction = self.fc_decoder([z, y_pred_site, y_pred_sex], training=False)
                ...
                y_pred_site_mean_over_batches=tf.reduce_mean(y_pred_site,axis=0, keepdims=True) #z_confounded
                z_mean_over_batches=tf.reduce_mean(fc_z, axis=0, keepdims=True) # should do the mean over all subjects in batch
                ...
                z_flatten = tf.keras.layers.Flatten()(fc_z-z_mean_over_batches)
                z_conf_flatten_site = (y_pred_site-y_pred_site_mean_over_batches) #z_confounded
        ...
        history = autoencoder_2.fit(x=[X_train, env_train], y=[y_train_site_multiclass, y_train_sex_multiclass],
                        epochs=epochs,
                        batch_size=batch,
                        shuffle=True, 
                        validation_data=([X_val, env_val], [y_val_site_multiclass, y_val_sex_multiclass]))
        ...
        sup_loss = autoencoder_2.history.history['clf_loss_site']
        sup_val_loss = autoencoder_2.history.history['val_clf_loss_site']
        fold_results.loc[i, 'train_clf_loss'] = sup_loss[-1]
        fold_results.loc[i, 'val_clf_loss'] = sup_val_loss[-1]
        
        sup_loss_sex = autoencoder_2.history.history['clf_loss_sex']
        sup_val_loss_sex = autoencoder_2.history.history['val_clf_loss_sex']
        
        plt.figure()
        plt.plot(epochs, sup_loss, 'b', label='Training clf loss (site)')
        plt.plot(epochs, sup_val_loss, 'g', label='Validation clf loss (site)')
        plt.plot(epochs, sup_loss_sex, 'r', label='Training clf loss (sex)')
        plt.plot(epochs, sup_val_loss_sex, 'orange', label='Validation clf loss (sex)')
        plt.title('Training and validation clf loss (site & sex)')
        plt.legend()
        plt.show()
        ...
        y_train_sites =  pd.DataFrame(y_train_site_multiclass).idxmax(axis=1).values
        y_val_sites = pd.DataFrame(y_val_site_multiclass).idxmax(axis=1).values
        sites = np.unique(y_train_sites)
        
        
        fc_train_encoded, h1_train, y_train_sites_pred, y_train_sex_pred = FC_encoder.predict(X_train)
        env_train_encoded = ENV_encoder.predict(env_train)
        z_train = fusion.predict([fc_train_encoded, env_train_encoded])      
        
        fc_train_decoded = fc_decoder.predict([z_train, y_train_sites_pred, y_train_sex_pred])
        
        fc_val_encoded,  h1_val , y_val_sites_pred, y_val_sex_pred = FC_encoder.predict(X_val)
        env_val_encoded = ENV_encoder.predict(env_val)
        z_val = fusion.predict([fc_val_encoded, env_val_encoded])      
        
        fc_val_decoded = fc_decoder.predict([z_val, y_val_sites_pred, y_val_sex_pred])
        
        #____Site classification based on softmax layer
      
        fold_results.loc[i,'softmax_site_auc_roc'] = roc_auc_score(y_val_site_multiclass, y_val_sites_pred, average='macro',multi_class='ovo')
        ...
        fold_results.loc[i,'z_site_auc_roc'] = roc_auc_score(y_val_site_multiclass, y_val_pred, average='macro',multi_class='ovo')
        ...
        fold_results.loc[i,'fc_z_site_auc_roc'] = roc_auc_score(y_val_site_multiclass, y_val_pred, average='macro',multi_class='ovo')
        ...
        fold_results.loc[i,'env_z_site_auc_roc'] = roc_auc_score(y_val_site_multiclass, y_val_pred, average='macro',multi_class='ovo')
        ...
        fold_results.loc[i,'env_site_auc_roc'] = roc_auc_score(y_val_site_multiclass, y_val_pred, average='macro',multi_class='ovo')
        ...
        fold_results.loc[i,'h1_site_auc_roc'] = roc_auc_score(y_val_site_multiclass, y_val_pred, average='macro',multi_class='ovo')
