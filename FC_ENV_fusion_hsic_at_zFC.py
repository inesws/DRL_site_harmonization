...
output_base_name = "fc_z_site_hsic_gamma15_alpha1"
...
def _hsic_loss(latent, confounder, sigma = None):
    ...
    K = _rbf_kernel(latent_centered, sigma)
    L = _rbf_kernel(conf_centered, sigma)
...
def _safe_auc_f1_multiclass(y_true_onehot, y_pred_proba, labels_for_f1):
    ...
    y_pred = pd.DataFrame(y_pred_proba).idxmax(axis=1).values
    f1 = f1_score(y_true_onehot, y_pred, labels=labels_for_f1, average="macro")
    return auc, f1
...
class AE(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            hsic_loss = _hsic_loss(fc_z, y_pred, None)
