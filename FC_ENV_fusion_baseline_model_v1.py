# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 10:42:55 2024
...
save_results = "/home/isampaio/Desktop/Ines/DRL_site/results/baseline/"
...
for r in range(0,2):
    ...
    for i, (train_ind, val_ind) in enumerate(skf.split(stacked_FC_matrices,labels.iloc[:,0].values)):
        ...
        fold_results.to_csv(os.path.join(save_results, "FC_ENV_fusion_baseline_model.csv"))



fold_results.iloc[i+1,:]=fold_results.iloc[0:10,:].mean(axis=0)

fold_results.T.to_csv(os.path.join(save_results, "FC_ENV_fusion_baseline_model.csv"))
